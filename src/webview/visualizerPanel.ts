 ...
 */
export class VisualizerPanel implements vscode.WebviewViewProvider {
  static readonly viewType = 'musicPlayer.visualizer';
  static readonly containerId = 'musicPlayerPanel';

  private view: vscode.WebviewView | undefined;
  private _onCommand: ((cmd: string, data?: any) => void) | undefined;
  ...
  onCommand(handler: (cmd: string, data?: any) => void): void {
    this._onCommand = handler;
  }

  show(): void {
    void vscode.commands.executeCommand(
      `workbench.view.extension.${VisualizerPanel.containerId}`,
    );
  }
  ...
  private buildHtml(webview: vscode.Webview): string {
    const barCount = vscode.workspace
      ...
      .get<number>('visualizerBars', 80);

    const barColor = vscode.workspace
      .getConfiguration('musicPlayer')
      .get<string>('visualizerColor', '#ffffff');

    const backgroundUri = webview.asWebviewUri(
      vscode.Uri.joinPath(this.context.extensionUri, 'music-visualizr', 'public', 'visualizer-bg.png'),
    );
