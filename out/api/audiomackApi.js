...
exports.AudiomackApi = exports.AudiomackAuth = void 0;
...
/**
 * Lightweight Audiomack API wrapper used by the player UI.
 * Methods that are not currently supported by Audiomack playback endpoints
 * are implemented as safe no-ops to keep the UI stable.
 */
class AudiomackApi {
    constructor(auth) {
        this.auth = auth;
    ...
    async getPlaybackState() {
        // Audiomack does not expose a Spotify-like "current playback state" endpoint.
        // Return null so UI falls back to idle behavior.
        return null;
    }
    async play(_uri) { }
    async pause() { }
    async next() { }
    async prev() { }
    async setVolume(_percent) { }
    async setShuffle(_state) { }
    async seek(_positionMs) { }
    async search(query, limit = 20) {
        ...
        const headers = await this.auth.signedHeaders('GET', url, params);
        try {
            ...
            const items = Array.isArray(data?.results)
                ...
                : Array.isArray(data?.data)
                    ? data.data
                    : [];
            return items.map(item => this.mapTrack(item)).filter(Boolean);
        }
        catch {
            return [];
        }
    ...
    mapTrack(item) {
        if (!item)
            return null;
        const id = String(item.id ?? item.track_id ?? item.slug ?? '');
        const title = String(item.title ?? item.name ?? '').trim();
        if (!id || !title)
            return null;
        const artist = String(item.artist ?? item.artist_name ?? item.uploader?.name ?? '').trim();
        const album = String(item.album ?? item.album_name ?? '').trim();
        const cover = String(item.image ?? item.artwork_url ?? item.thumbnail ?? '').trim();
        const durationRaw = Number(item.duration ?? item.duration_ms ?? 0);
        const durationMs = Number.isFinite(durationRaw) && durationRaw > 0
            ? (durationRaw > 10000 ? durationRaw : Math.round(durationRaw * 1000))
            : 0;
        const uri = String(item.url ?? item.permalink ?? item.stream_url ?? '');
        return {
            id,
            name: title,
...
exports.AudiomackApi = AudiomackApi;
//# sourceMappingURL=audiomackApi.js.map